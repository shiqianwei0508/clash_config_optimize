...
- 支持自定义端口和基础配置
- "🚀 节点选择"组使用select类型支持手动选择节点
- "🌐 所有节点 (自动选择)"组使用url-test类型自动选择最优节点
...
2. **代理节点**：所有解析成功的代理节点
3. **代理组**：
   - 🚀 节点选择（主节点选择组，类型：select，手动选择）
   - 🇺🇸 美国节点（按国家分组，类型：url-test）
   - 🇭🇰 香港节点（按国家分组，类型：url-test）
   - 🇯🇵 日本节点（按国家分组，类型：url-test）
   - 🌐 其他节点（按国家分组，类型：url-test）
   - 🌐 所有节点 (自动选择)（包含所有节点，类型：url-test）
   - 📺 流媒体（专用代理组，类型：url-test）
   - 🌍 全球直连（专用代理组，类型：url-test）
   - 🛡️ 隐私保护（专用代理组，类型：url-test）
4. **规则配置**：常用网站和服务的分流规则
